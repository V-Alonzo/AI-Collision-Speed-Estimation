...
--   • NO se crea tabla USUARIO: se referencia sys_users.id_user mediante FK.
--   • NO se crean CAT_TIPO_HECHO ni CAT_TIPO_VIA: se referencian
--     rat_tipos_hecho.id y rat_tipos_via.id ya existentes en la BD base.
--   • Se crea RAT_PERITO_PERFIL para extender sys_users con datos del perito.
--   • Los catálogos restantes son propios del módulo RAT (no existen en base).
-- =============================================================================
...
-- =============================================================================
-- 1. EXTENSIÓN DE USUARIO — Perfil del perito
-- =============================================================================
-- Extiende sys_users (de la plataforma base) con datos específicos del perito.
-- La pantalla "Mi Perfil" muestra: teléfono, cédula profesional, especialidad,
-- número de empleado, calificación y fecha de alta.

CREATE TABLE RAT_PERITO_PERFIL (
  id_user           INT           NOT NULL  COMMENT 'FK → sys_users.id_user (1:1)',
  telefono          VARCHAR(20)   NULL,
  cedula_profesional VARCHAR(30)  NULL,
  especialidad      VARCHAR(200)  NULL      COMMENT 'Ej: Reconstrucción de Accidentes Viales',
  numero_empleado   VARCHAR(30)   NULL      COMMENT 'Ej: CESVI-MX-1048',
  calificacion      DECIMAL(3,1)  NULL      COMMENT 'Rating 0.0 – 5.0',
...
) ENGINE=InnoDB COMMENT='Extiende sys_users con datos propios del perito RAT';

-- =============================================================================
-- 2. CATÁLOGOS PROPIOS DEL MÓDULO RAT
--    (CAT_TIPO_HECHO y CAT_TIPO_VIA NO se crean aquí — ya existen como
--     rat_tipos_hecho y rat_tipos_via en la plataforma base)
-- =============================================================================
...
CREATE TABLE RAT_CAT_PARTE_VEHICULO (
  ...
  subzona       VARCHAR(50)   NULL     COMMENT 'Frontal / Media / Trasera',
...
-- 4. INCIDENTE
-- =============================================================================
-- La pantalla "Expedientes RAT" muestra: No.Siniestro, fecha, hora,
-- tipo de hecho, vehículo (marca+modelo+año), perito, estado, vel calculada,
-- exceso de velocidad. Todo esto se construye con JOINs desde aquí.
-- id_usuario_perito → sys_users.id_user

CREATE TABLE RAT_INCIDENTE (
  id                 BIGINT       NOT NULL AUTO_INCREMENT,
  numero_siniestro   VARCHAR(100) NOT NULL COMMENT 'Ej: RAT-2026-024',
  fecha_hecho        DATE         NOT NULL,
  hora_hecho         TIME         NULL,
  tipo_hecho_id      INT          NOT NULL COMMENT 'FK → rat_tipos_hecho.id (plataforma base)',
  id_usuario_perito  INT          NOT NULL COMMENT 'FK → sys_users.id_user',
  estado             TINYINT      NOT NULL DEFAULT 0
                       COMMENT '0=Abierto / 1=En revisión / 2=Finalizado',
  created_at         DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at         DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT pk_incidente          PRIMARY KEY (id),
  CONSTRAINT uq_incidente_num      UNIQUE (numero_siniestro),
  CONSTRAINT fk_inc_tipo_hecho     FOREIGN KEY (tipo_hecho_id)     REFERENCES rat_tipos_hecho(id),
...
) ENGINE=InnoDB COMMENT='Expediente RAT. estado: 0=Abierto, 1=En revisión, 2=Finalizado';

-- =============================================================================
-- 5. UBICACIÓN DE LA VÍA
-- =============================================================================
-- Corrección v4: se eliminaron campos que no corresponden a la ubicación:
--   ✗ estado_neumatico_id  → movido a RAT_TABLA_MU (lookup técnico)
-- Se mantienen únicamente características físicas y ambientales de la vía.
...
CREATE TABLE RAT_UBICACION_VIA (
  ...
  tipo_via_id                    INT           NULL     COMMENT 'FK → rat_tipos_via.id (plataforma base)',
  ...
  clima_id                       INT           NULL,
  -- Coeficientes para cálculo (se derivan de TABLA_MU pero se guardan aquí)
  mu_coeficiente_adherencia      DECIMAL(5,4)  NULL     COMMENT 'Lookup RAT_TABLA_MU',
  ...
  radio_curva_m                  DECIMAL(8,2)  NULL     COMMENT 'Para velocidad de derrape/vuelco',
  peraltaje_pct                  DECIMAL(6,3)  NULL     COMMENT 'Para velocidad de derrape',
  ...
  CONSTRAINT fk_ubic_tipo_via       FOREIGN KEY (tipo_via_id)                REFERENCES rat_tipos_via(id),
...
CREATE TABLE RAT_INCIDENTE_VEHICULO (
  ...
  color_id      INT         NULL,
  estado_neumatico_id INT   NULL  COMMENT 'Condición del neumático al momento del siniestro',
  ...
  COMMENT='estado_neumatico_id aquí: refleja el estado del neumático del vehículo al momento del hecho, que afecta el coeficiente mu';
...
CREATE TABLE RAT_DEFORMACION_MEDICION (
  ...
  l_ancho_contacto_m    DECIMAL(6,3) NULL COMMENT 'CALCULADA o medida en campo',
  angulo_fpi_grados     DECIMAL(6,2) NULL COMMENT 'Ángulo de la fuerza principal de impacto',
...
-- -----------------------------------------------------------------------------
-- Cálculo McHenry completo + bloque de energía de vuelco (v3.0)
-- La pantalla muestra: velocidad calculada y exceso → campos velocidad_final_kmh
-- y exceso_velocidad son los que alimentan la lista de expedientes.
...
CREATE TABLE RAT_CALCULO_VELOCIDAD (
  ...
  a_rigidez_n_m               DECIMAL(12,3) NULL  COMMENT 'Lookup RAT_TABLA_RIGIDEZ_AB',
  b_rigidez_n_m2              DECIMAL(12,3) NULL  COMMENT 'Lookup RAT_TABLA_RIGIDEZ_AB',
  ...
  -- Energía de vuelco (aplica cuando el vehículo termina volcado)
  ...
  centro_gravedad_m           DECIMAL(5,3)  NULL  COMMENT 'Desnorm. desde RAT_VEHICULO.centro_gravedad_m',
  ...
  ebs_m_s                     DECIMAL(8,4)  NULL  COMMENT 'CALCULADA: √(2 × E_total / masa_total)',
  ...
  -- Resultado final → alimenta la columna "Vel. Calculada" y "Exceso" en la UI
  velocidad_final_kmh         DECIMAL(8,2)  NULL  COMMENT 'Resultado reportado en el dictamen',
  exceso_velocidad            TINYINT(1)    NULL  COMMENT '0=no / 1=sí',
  delta_exceso_kmh            DECIMAL(8,2)  NULL  COMMENT 'vel_final − vel_max_permitida',
...
CREATE TABLE RAT_CONCLUSION (
  ...
  validado_por_id        INT        NULL     COMMENT 'FK → sys_users.id_user',
...
-- =============================================================================
-- 10. PLANTILLA PARA GENERACIÓN AUTOMÁTICA DEL DOCUMENTO WORD
-- =============================================================================
...
CREATE TABLE RAT_PLANTILLA_PARRAFO (
  ...
  condicion       VARCHAR(200) NULL     COMMENT 'Ej: aplica_vuelco=1',
...
INSERT INTO RAT_PLANTILLA_VARIABLE (clave, descripcion, tabla_origen, columna_origen, formato) VALUES
  ('{{marca}}',                'Marca del vehículo',                      'RAT_VEHICULO',            'marca',                           'mayusculas'),
  ('{{submarca}}',             'Submarca / tipo del vehículo',            'RAT_VEHICULO',            'submarca',                        'mayusculas'),
  ('{{color}}',                'Color del vehículo',                      'RAT_CAT_COLOR',           'nombre',                          'texto'),
  ('{{placas}}',               'Número de placas',                        'RAT_INCIDENTE_VEHICULO',  'numero_placas',                   'mayusculas'),
  ('{{anio_modelo}}',          'Año modelo del vehículo',                 'RAT_VEHICULO',            'anio_modelo',                     'texto'),
  ('{{vin}}',                  'Número de serie (VIN)',                    'RAT_VEHICULO',            'vin',                             'mayusculas'),
  ...
  ('{{tipo_hecho}}',           'Tipo de hecho de tránsito',               'rat_tipos_hecho',         'nombre',                          'texto'),
  ('{{km_punto}}',             'Kilómetro del punto del hecho',           'RAT_UBICACION_VIA',       'km_punto',                        'texto'),
  ('{{municipio}}',            'Municipio del hecho',                     'RAT_UBICACION_VIA',       'municipio',                       'texto'),
  ('{{estado_republica}}',     'Estado de la república',                  'RAT_UBICACION_VIA',       'estado_republica',                'texto'),
  ('{{tipo_via}}',             'Tipo de vía',                             'rat_tipos_via',           'nombre',                          'texto'),
  ('{{velocidad_max_kmh}}',    'Velocidad máxima permitida',              'RAT_UBICACION_VIA',       'velocidad_maxima_permitida_kmh',  'texto'),
  ('{{numero_siniestro}}',     'Número de siniestro',                     'RAT_INCIDENTE',           'numero_siniestro',                'mayusculas'),
  ('{{numero_formato}}',       'Número de formato del reporte',           'RAT_REPORTE',             'numero_formato',                  'texto'),
  ...
  ('{{e_deformacion_julios}}', 'Energía de deformación (J)',              'RAT_CALCULO_VELOCIDAD',   'e_deformacion_julios',            'decimal_3'),
  ('{{e_vuelco_julios}}',      'Energía de vuelco (J)',                   'RAT_CALCULO_VELOCIDAD',   'e_vuelco_julios',                 'decimal_3'),
  ('{{e_total_julios}}',       'Energía total (J)',                       'RAT_CALCULO_VELOCIDAD',   'e_total_julios',                  'decimal_3'),
  ('{{narracion_hechos}}',     'Narración dinámica del hecho',            'RAT_NARRATIVA_DINAMICA',  'narracion_hechos',                'texto'),
  ('{{objeto_involucrado}}',   'Objeto fijo / vehículo involucrado',      'RAT_NARRATIVA_DINAMICA',  'objeto_involucrado',              'texto'),
  ('{{perito_nombre}}',        'Nombre del perito elaborador',            'sys_users',               'name',                            'mayusculas'),
  ('{{perito_cedula}}',        'Cédula profesional del perito',           'RAT_PERITO_PERFIL',       'cedula_profesional',              'texto'),
  ('{{perito_especialidad}}',  'Especialidad del perito',                 'RAT_PERITO_PERFIL',       'especialidad',                    'texto'),
  ('{{fecha_elaboracion}}',    'Fecha de elaboración del documento',      'RAT_REPORTE',             'fecha_elaboracion',               'fecha_larga'),
  ('{{ubicacion_taller}}',     'Taller donde se encuentra el vehículo',   'RAT_REPORTE',             'ubicacion_taller',                'texto');
...
-- RESUMEN v1.1
-- ─────────────────────────────────────────────────────────────────────────────
-- Tablas eliminadas vs v3:
--   ✗ USUARIO           → se usa sys_users (plataforma base)
--   ✗ CAT_TIPO_HECHO    → se usa rat_tipos_hecho (plataforma base)
--   ✗ CAT_TIPO_VIA      → se usa rat_tipos_via (plataforma base)
--
-- Tablas nuevas vs v3:
--   ✚ RAT_PERITO_PERFIL → extiende sys_users con datos del perito
--
-- Correcciones de ubicación de campos:
--   ✎ estado_neumatico_id  UBICACION_VIA → RAT_INCIDENTE_VEHICULO
--     (el estado del neumático es del vehículo, no de la vía)
--   ✎ estado_republica renombrado (evitar colisión con palabra reservada)
--
-- Todas las tablas propias llevan prefijo RAT_ para convivir con sys_*
--
-- Total tablas: 38
--   Perfil perito    :  1  (RAT_PERITO_PERFIL)
--   Catálogos propios: 25  (RAT_CAT_*)
--   Tablas técnicas  :  3  (RAT_TABLA_*)
--   Transaccionales  : 10  (RAT_INCIDENTE → RAT_CONCLUSION)
--   Plantilla        :  4  (RAT_PLANTILLA*)
-- Tablas referenciadas de plataforma base: sys_users, rat_tipos_hecho, rat_tipos_via
-- =============================================================================
